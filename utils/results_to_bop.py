...
def write_bop_csv(output_root, csv_path):
    ...
    for scene_path in scenes:
        ...
        for im_id_str, entries in gt_data.items():
            ...
            for entry in entries:
                R_str = ' '.join(f"{val:.6f}" for val in entry['cam_R_m2c'])
                t_str = ' '.join(f"{val:.6f}" for val in entry['cam_t_m2c'])
                row = [
                    scene_id,
                    im_id,
                    int(entry['obj_id']),
    ...
    print(f"[INFO] BOP-compatible CSV with compact R and t saved to {csv_path}")
